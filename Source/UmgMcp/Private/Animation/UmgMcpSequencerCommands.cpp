...
TSharedPtr<FJsonObject> FUmgMcpSequencerCommands::AppendTimeSlice(const TSharedPtr<FJsonObject>& Params)
{
    ...
    for (const TSharedPtr<FJsonValue>& WidgetVal : *WidgetsPtr)
    {
        ...
        const TSharedPtr<FJsonObject> PropertiesObj = *PropertiesObjPtr;
...
TSharedPtr<FJsonObject> FUmgMcpSequencerCommands::DeleteWidgetKeys(const TSharedPtr<FJsonObject>& Params)
{
    ...
    auto RemoveIndicesFromChannelData = [](auto ChannelData, const TArray<int32>& Indices)
    {
        for (int32 Index = Indices.Num() - 1; Index >= 0; --Index)
        {
            const int32 KeyIndex = Indices[Index];
    ...
    if (UMovieSceneTrack* Track = MovieScene->FindTrack(UMovieSceneFloatTrack::StaticClass(), WidgetGuid, FName(*PropertyName)))
    {
        ...
        for (UMovieSceneSection* Section : Track->GetAllSections())
        {
            if (UMovieSceneFloatSection* FloatSection = Cast<UMovieSceneFloatSection>(Section))
            {
                ...
                if (Indices.Num() > 0)
                {
                    RemoveIndicesFromChannelData(FloatSection->GetChannel().GetData(), Indices);
    ...
    if (UMovieSceneTrack* Track = MovieScene->FindTrack(UMovieSceneColorTrack::StaticClass(), WidgetGuid, FName(*PropertyName)))
    {
        ...
        for (UMovieSceneSection* Section : Track->GetAllSections())
        {
            if (UMovieSceneColorSection* ColorSection = Cast<UMovieSceneColorSection>(Section))
            {
                ...
                if (Indices.Num() > 0)
                {
                    RemoveIndicesFromChannelData(ColorSection->GetRedChannel().GetData(), Indices);
                    RemoveIndicesFromChannelData(ColorSection->GetGreenChannel().GetData(), Indices);
                    RemoveIndicesFromChannelData(ColorSection->GetBlueChannel().GetData(), Indices);
                    RemoveIndicesFromChannelData(ColorSection->GetAlphaChannel().GetData(), Indices);
                    RemovedKeys += Indices.Num();
                }
            }
        }
        ...
        for (UMovieSceneSection* Section : Track->GetAllSections())
        {
            if (UMovieSceneDoubleVectorSection* VectorSection = Cast<UMovieSceneDoubleVectorSection>(Section))
            {
                ...
                if (Channels.Num() >= 2)
                {
                    ...
                    if (Indices.Num() > 0)
                    {
                        RemoveIndicesFromChannelData(Channels[0]->GetData(), Indices);
                        RemoveIndicesFromChannelData(Channels[1]->GetData(), Indices);
                        RemovedKeys += Indices.Num();
                    }
                }
            }
