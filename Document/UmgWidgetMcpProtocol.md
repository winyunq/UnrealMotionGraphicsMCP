...
3. 写操作只做并集覆盖、追加和重排。`create_widget` 只创建缺失控件；`set_widget_properties` 只覆盖传入属性，不删除未提及属性；`reorder_widget_tree` 只移动已存在的同父级控件顺序；`apply_layout` 应按 append/upsert 语义处理布局。
...
| `reorder_widget_tree(tree, root?)` | 并集式重排同父级控件顺序；未提及控件保持相对顺序，不创建不删除。 |
