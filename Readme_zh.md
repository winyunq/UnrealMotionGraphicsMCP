...
## 蓝图 (Blueprint) API 实现状态（过渡期）

BlueCode 现在已把默认的注意力压缩语义读取与 round-trip/debug 证据分层；union 写入支持图 revision、source map、dry-run 计划、LCS 右锚点插入、安全追加和失败回滚。但它仍处于过渡期：嵌套分支、循环、Sequence/Switch、Latent 节点和不可规约控制流的完整结构化 CFG 读写仍是 v2 工作。底层节点命令只是兼容执行原语，不等于 BlueCode 语言本身。
